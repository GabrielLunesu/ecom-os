...
visible through `origin`; A03 is now visible through `origin` but not integration-gate
verified. Fresh A00 checks also show A05 still has focused mypy failures, A08 needs an
accepted authenticated Finance router mount before its local UI can read live data, A04
