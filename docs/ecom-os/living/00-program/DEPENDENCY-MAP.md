...
| A03–A08 | A02 | Trace context, tool invocation, durable action, job/event ports | Local durable-core draft, not published | Blocks verified tool/action integration | A02 local `backend/app/{actions,events,jobs,traces}/**`, models, migration `a02d1e2f3a4b`, and `test_durable_core.py`; targeted tests passed `5 passed`; focused ruff/mypy pass |
| A05/A07/A08 | A03 | HermesBridge session/run/channel contracts and capability model | Local commit, not published; owner docs stale | Blocks Hermes-backed workflows and native delivery | A03 local targeted tests passed `57 passed`; no real Hermes probe |
