...
|---|---|---|---|---|---|
| A02–A09 | A01 | Identity, request context, common money/time/error/ID contracts, route registration convention | Local ready-for-integration claim at `64ca874`, not published to `origin`, owner docs stale for current head | Blocks integration beyond isolated fakes | A01 local ruff/mypy pass at `64ca874`; previous owner full-suite evidence at `f92adbb` was `628 passed, 1 xfailed`; A00 full pytest attempts at `64ca874` failed before collection due host data volume full/temp-file failure; no `origin/agent/a01-foundation` ref observed |
| A03–A08 | A02 | Trace context, tool invocation, durable action, job/event ports | Local durable-core/webhook/activity/queue-worker draft, not published; source boundary widened | Blocks verified tool/action integration | A02 local `backend/app/{actions,events,jobs,traces}/**`, `api/{ecom_webhooks,board_webhooks,activity}.py`, `services/{queue_worker,webhooks/dispatch}.py`, models, migration `a02d1e2f3a4b`, migration verifier, broad legacy service/connector/CS diffs, and focused tests; targeted tests passed `47 passed`; focused ruff/mypy pass |
| A05/A07/A08 | A03 | HermesBridge session/run/channel contracts and capability model | Pushed `origin/agent/a03-hermes-runtime` at `4108248`; additional local post-push edits unpublished; owner docs stale; programme-file ownership collision; real conformance gate RED until endpoint supplied | Blocks Hermes-backed workflows and native delivery | A03 focused pushed-branch checks passed `97 + 3` tests; focused ruff/mypy pass; compatibility matrix/drift guard added; conformance CLI exits `2` BLOCKED without real Hermes |
...
| A09 | All builders | Ready branches with tests, migrations, accepted interfaces, and current living docs | Not available; local readiness tooling exists but has static failures | Blocks integration queue | A00 and A03 refs are published on `origin`, but A03 is still not ready and has local post-push edits; A09 branch-readiness tool reports all builders NOT READY; A01/A04/A06/A09 commits and A02/A03/A05/A07/A08 diffs are local only |
...
| A04 | A02/A03/A01/A06 | Durable inbox/action/event ports, tool catalog registration, common types/client workflow, UI route/state patterns | Requested locally, not accepted | Blocks connector webhook/read-tool/UI integration | Local A04 `INTERFACES.md` names these dependencies; A04 UI state/nav request reconciled as IR-028 |
...
| A08 | A01/A02/A03/A04/A07/A06/A09 | Economics source port, trace/evidence/jobs, narration/channel delivery, task/research inputs, authenticated `/finance` backend mount, and `/finance` route/nav | Requested locally, not accepted | Blocks finance and daily brief integration | Local A08 `INTERFACES.md` names these dependencies; delivery-packet regression, backend checks, and targeted Finance frontend checks now pass locally; backend mount request reconciled as IR-027 |
...
A01/A03/A04/A05/A06/A07/A08 now name concrete requests locally or in pushed branch docs,
while A01/A02/A04/A05/A06/A07/A08/A09 still have source/config/test evidence that is not
visible through `origin`; A03 is visible through `origin` at `4108248` but has local
post-push drift and is not integration-gate verified. Fresh A00 checks also show A01's
latest full pytest could not start because the host data volume is full, A02 has broadened
into non-A02-owned service/connector files, A05 still has focused mypy failures, A08 needs
an accepted authenticated Finance router mount before its local UI can read live data, A04
has staged A00 programme-file edits to clean up, and A09's new readiness tool has ruff/mypy
failures.
