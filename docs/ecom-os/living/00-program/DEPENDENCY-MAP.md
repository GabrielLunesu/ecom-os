...
| A05/A07/A08 | A03 | HermesBridge session/run/channel contracts and capability model | Local commit `4108248`, not published; owner docs stale; programme-file ownership collision; real conformance gate RED until endpoint supplied | Blocks Hermes-backed workflows and native delivery | A03 local targeted tests passed `100 passed`; focused ruff/mypy pass; compatibility matrix/drift guard added; no real Hermes probe |
...
also show A05 still has focused mypy failures, A08 needs an accepted authenticated Finance
router mount before its local UI can read live data, and A09's new readiness tool has
ruff/mypy failures.
