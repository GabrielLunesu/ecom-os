...
| A03–A08 | A02 | Trace context, tool invocation, durable action, job/event ports | Local durable-core/webhook/activity draft, not published | Blocks verified tool/action integration | A02 local `backend/app/{actions,events,jobs,traces}/**`, `api/{ecom_webhooks,board_webhooks,activity}.py`, `services/webhooks/dispatch.py`, models, migration `a02d1e2f3a4b`, and focused tests; targeted tests passed `34 passed`; focused ruff/mypy pass |
| A05/A07/A08 | A03 | HermesBridge session/run/channel contracts and capability model | Local commit `2a1ce14`, not published; owner docs stale; real conformance gate RED until endpoint supplied | Blocks Hermes-backed workflows and native delivery | A03 local targeted tests passed `92 passed`; focused ruff/mypy pass; no real Hermes probe |
