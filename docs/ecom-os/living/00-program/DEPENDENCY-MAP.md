...
| A03–A08 | A02 | Trace context, tool invocation, durable action, job/event ports | Local durable-core/webhook/activity draft, not published | Blocks verified tool/action integration | A02 local `backend/app/{actions,events,jobs,traces}/**`, `api/{ecom_webhooks,activity}.py`, models, migration `a02d1e2f3a4b`, `test_durable_core.py`, and `test_realtime_webhook.py`; targeted tests passed `14 passed`; focused ruff/mypy pass |
| A05/A07/A08 | A03 | HermesBridge session/run/channel contracts and capability model | Local commit `689cf10`, not published; owner docs stale; real conformance gate RED until endpoint supplied | Blocks Hermes-backed workflows and native delivery | A03 local targeted tests passed `84 passed`; focused ruff/mypy pass; no real Hermes probe |
...
| All route owners | A06 | UI token/component/state contract | Local source/design draft, not published | Blocks final UI integration, but route owners can use local placeholders | A06 local theme/mobile/state/component-lab/card/a11y primitives inspected; typecheck passed; Vitest passed `150 passed`; whole-project eslint fails in A07-owned brand page |
| A09 | All builders | Ready branches with tests, migrations, accepted interfaces, and current living docs | Not available | Blocks integration queue | Only A00 refs are published on `origin`; A01/A03/A04/A09 commits and A02/A05/A06/A07/A08 diffs are local only |
...
| A09 | A01/A02/A04/A05/A07/A08/A09 | Migration graph from local implementation drafts | Not accepted | Blocks any integration of new migrations | Branch-local heads observed: A01 `a01_0001_identity`, A02 `a02d1e2f3a4b`, A04 `a04commerce01`, A05 `a05h1f6a0006`, A08 `a08_001_metric_snapshots`, A09 `a09c1d2e3f40`; A07 draft revises older `e2f9c6b4a1d3` |
