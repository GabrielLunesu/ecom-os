...
last_verified_against: 0e0badf6a871f8fff43ac7b01b31fcced12a9aae
---
...
| A02 | `agent/A02-durable-core` | `agent/a02-trace-ledger` at `3909904`; local uncommitted durable-core service/model/migration/docs plus realtime-webhook acceptance draft | local durable-core + webhook acceptance draft | Durable core | Unpublished source; A02 ports not accepted by consumers yet; integration/E2E wiring still pending; provider raw-body signature remains future A04/A02 work | Local `backend/app/{actions,events,jobs,traces}/**`, `api/ecom_webhooks.py`, `models/{actions,events,traces}.py`, migration `a02d1e2f3a4b`; targeted tests `12 passed`; ruff and mypy pass; branch-local Alembic head `a02d1e2f3a4b` |
| A03 | `agent/A03-hermes-integration` | `agent/a03-hermes-runtime` at local commit `ea65611` plus uncommitted native/compat/health/catalog-MCP draft; no `origin/agent/a03-hermes-runtime` ref observed | local bridge/catalog/invoker/run/channel/conformance commit + native/compat draft | Hermes bridge/chat | Unpublished source; real pinned-Hermes conformance evidence still missing; OpenClaw is now explicitly compat/dev only; local owner docs contain stale status/commit text | Local `backend/app/hermes/**`, `backend/app/tools/**`, `mcp_server/catalog_server.py`; targeted v2/native/compat/catalog tests `77 passed`; ruff and mypy pass |
...
| A05 | `agent/A05-customer-service` | `agent/a05-finance` at `3909904`; local uncommitted docs plus lifecycle/lease/action-policy/shadow-draft/action-proposal/action-execution/approval source, migrations, and tests | local discovery + invariant/lease/action-policy/shadow-draft/proposal/execution/approval draft | CS/autonomy | Unpublished source; blocked on accepted A02/A03/A04/A06 contracts for production writes; owner docs omit the newest `a05h1f6a0006` migration in verification | Local CS lifecycle/proposal/execution/approval services, migrations through `a05h1f6a0006`; targeted regression suite `64 passed`; focused ruff pass; branch-local Alembic head `a05h1f6a0006` |
...
`origin/agent/a00-orchestrator` and `origin/coordination/program`, both then at `0e0badf`;
...
A02/A03/A05/A06/A07/A08 have unpublished worktree diffs.
...
services, a migration, realtime-email durable acceptance, owner docs, and focused checks
passing; A03 has HermesBridge/tool catalog/invoker plus background-run/channel/conformance
fixtures, an honest native blocked stub, OpenClaw compat/dev transport, and catalog MCP
server tests passing, but still no real pinned-Hermes conformance; A04 has a local
committed connector slice with targeted tests passing; A05 has ticket lifecycle, CS-loop
lease, action-policy, WISMO shadow-draft, action-proposal, local attempt-execution, and
approval migration evidence passing; A06 has theme/mobile-nav,
