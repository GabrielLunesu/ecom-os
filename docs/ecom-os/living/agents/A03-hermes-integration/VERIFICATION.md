...
| Compatibility matrix (Runtime §17) | `hermes-integration/COMPATIBILITY.md` + `conformance/*.json` | pinned baseline + catalog hash recorded; conformance status BLOCKED (honest) | updated when a real runtime is probed |
...
| Compatibility-matrix drift guard | `uv run python -m pytest tests/test_catalog_manifest_pinned.py -q` | **3 passed** — live catalog compatibility hash + capability model match pinned `hermes-integration/conformance/*.json` | n/a |
