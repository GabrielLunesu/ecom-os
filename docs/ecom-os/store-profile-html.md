...
and free-form **brand facts**. The CS flow uses these real values — set the explicit
`tracking_url` to the real tracking app page (for Chicago Outlet:
`https://chicagooutletshop.com/apps/trackyourorder`), the signature is `support_name`, and
`facts` are available to the agent. API: `PUT /ecom/stores/{id}/profile`. Multi-store: each
store carries its own profile.
...
## Verified live
A WISMO reply used the profile tracking URL + signature "Chicago Outlet Support", with
paragraphs/links — no myshopify URL, no guessed facts.
