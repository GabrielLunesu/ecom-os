...
Agent. It is not a Hermes fork, wrapper product, or replacement runtime. The canonical
contracts live in `docs/ecom-os/specs/`; do not revive assumptions from older documents.
...
## 13. Parallel-agent build protocol

When the parallel build programme is active, every agent MUST also follow
`docs/ecom-os/parallel-build/OPERATING-PROTOCOL.md` and the ownership map.

- Work only in the assigned branch/worktree.
- Read the complete normative set, programme state, all agents' `CURRENT.md` and
  `INTERFACES.md`, then the assigned handoff before changing code.
- Keep the assigned files in `docs/ecom-os/living/agents/` current. They are current-state
  manuals, not journals or changelogs.
- A00 is an independent auditor. A00 may inspect code and run checks, but may edit only
  `docs/ecom-os/living/00-program/**`.
- Builders do not edit another agent's owned source, global registry, lockfile, navigation,
  deployment file, or living-doc folder. Request an interface or registration instead.
- Domain agents export routers, schemas, tool manifests, and ports; designated shared-file
  owners perform central registration.
- Every builder pushes coherent checkpoints so A00 and A09 can inspect the branch.
- A branch is not ready for integration until its living docs, exact test evidence,
  migrations, failure states, and acceptance criteria match the implementation.
- Living documents never override an invariant, ADR, or accepted specification. Raise a
  decision request and write an ADR when normative behavior must change.
- Do not wait idle for another branch. Continue with typed ports, fakes, fixtures, and
  non-conflicting work while recording the precise dependency.

The assigned branch names, source ownership, route ownership, migration protocol, and
integration gates are normative for the programme and live under
